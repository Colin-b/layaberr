...
pipeline {
    ...
    stages {
        ...
        stage('Test') {
            steps {
                sh """
                nosetests test --exe --with-doctest --with-xunit --xunit-file test-results.xml --with-coverage --cover-erase --cover-package=pycommon_error/. --cover-min-percentage=92 --cover-html
                """
            }
        }
